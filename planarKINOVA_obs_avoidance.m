...
addpath(genpath('kinova_arm'))
load('saved_data\planarKINOVA_ee_plan.mat')
load('saved_data\planarKINOVA_navigation_params.mat')

%% obstacle parameters

obs_radius = navigation_params.x_obs_rad;
safe_rad = obs_radius*0.9;
...
%obs_location = [x_obs(1)+x_offset,x_obs(2)+y_offset,z_height];
obs_location = [x_obs(1)+ee_offset+x_offset,...
                x_obs(2)+y_offset,...
                z_height];
...
% obstacle
Cylinder1 = collisionCylinder(safe_rad, 0.4);
...
Cylinder2 = collisionCylinder(safe_rad+0.05, 0.4);
